          ...
          <div class="cont_table">
            <table class="table_cont">
              ...
              <template v-for="(gpu, i) in gpuTableData" :key="i">
                ...
                <tr v-for="(item, j) in gpu.configs.slice(1)" :key="j">
                  <td>{{ item.group }}</td>
                  <td>{{ item.bandwidth }}</td>
                  <td>{{ formatWithThousandSeparator(item.power) }}</td>
                  <td>{{ formatWithThousandSeparator(item.temp) }}</td>
                  <td>{{ formatWithThousandSeparator(item.noise) }}</td>
                </tr>
              </template>
            </table>
          </div>
      ...
      <div class="text2">
        <div class="text_select">
          ...
          <el-select
            class="el-select-type select_width"
            v-model="gpu_type1"
            :placeholder="$t('short_rule.cont4.text6')"
            @change="SelectGPU1"
          >
            <el-option
              v-for="(item, index) in options"
...
export default defineComponent({
  ...
  setup() {
    ...
    const options = ref([
      ...
      { name: '5060ti', cuda: '4608', men: '16', large: 1 },
