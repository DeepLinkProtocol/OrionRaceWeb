export default {
  ...
  home: {
    ...
    btn_title2: '还剩余 {short_term} GPU',
    cont2_text1: 'Orion算力竞赛的Token奖励总数为30亿枚，其中长租模式总共奖励为20亿DLC，短租模式总奖励10亿DLC奖励，持续180天时间，竞赛总共分为3次，每次持续60天时间，每次间隔5-10天时间',
    cont2_btn1: '长租模式',
    cont2_btn2: '短租模式',
    cont3: {
      text1: '总算力值',
  ...
  long_rule: {
    ...
    cont2: {
      title: '一.参与规则',
      text1_1: '参与猎户座算力竞赛的矿工，需要持有节点NFT（每个节点送3万个DLC空投,价值90美金,详细了解节点：',
      text1_2: '）。竞赛期间，一个GPU机器可以持有多个节点，奖励Token数量和节点数量是乘数关系。单个机器持有节点NFT上限是20个。',
      ...
      text5_1: 'CPU',
      text5_2: '满足云游戏的要求：Intel/AMD，主频3.0GHz或者睿频3.5GHz以上；每个GPU需要单独配置一个CPU,且可以流畅运行黑神话悟空.比如：Intel CPU 10代及以上的版本,AMD 5600及以上版本',
      text6_1: '硬盘',
    ...
    cont5: {
      ...
      text1: '猎户座算力竞赛的IDC长租模式GPU机器Token奖励总数为20亿枚，时间持续180天。竞赛总共分为3次。每次间隔5-10天时间。',
      text2: '竞赛一：每个类型GPU数量只有达到500个，竞赛才开始正式启动。',
      text3: '60天，每天600万枚，10％立马得到，90％ 180天解锁，每天解锁0.5%。',
      text4: '竞赛二：每个类型GPU数量只有达到1000个，竞赛才开始正式启动。',
      text5: '60天，每天800万枚，10％立马得到，90％ 180天解锁，每天解锁0.5%。',
      text6: '竞赛三：每个类型GPU数量只有达到1500个，竞赛才开始正式启动。',
      text7: '60天， 每天 1933万枚，10％立马得到，90％ 180天解锁，每天解锁0.5%。',
      text8: 'Listing交易所后，测试币可以1：1兑换主网币',
      text9: 'DeepLink采用非线性奖励算法',

      text10: '这个系统的奖励每天的总token数量是固定的，机器获得奖励与机器的算力值X以及机器加入挖矿的有效时间Y,以及机器的质押Token数量有关系，质押的Token越多奖励越多，但是不能呈现线性增长关系。0-10000个币奖励基数是一样的，当超过10000个币奖励与函数呈现关联.如果机器处于被租用状态算力值增加30%',
      text11: '奖励计算公式：',
      ...
      text1_4: '4）100000个币的奖励是质押10000个币的1.23026倍，即大约增加了23.03%，100万个币的奖励是质押10000个币的1.46052倍，即大约增加了46.05%，质押1000万个币的奖励是质押10000个币的1.69078倍，即大约增加了69.08%，质押10000万个币的奖励是质押10000个币的1.92103倍，即大约增加了92.10%',

      title2: '奖励提取：',
      text2_1: '每24小时可以提取一次，每次提取时，对于新产生的奖励，其中10%可立即提取，其中90%锁定180天，锁定的部分每次提取时可提取0.5%，180天后可全部提取',

     
      block_title1: '特别注意：',
      block_text1_1: '长租模式 需要先把GPU机器加入到DBC网路中，参考文档：',
      block_text1_2: '。然后矿工自己将机器租用后才可以加入到DLC的网络中。从而同时获得DBC和DLC两份收益，加入到DBC的网络中可以获得的DBC收益=DBC的奖励- 30%租金',
      block_title2: '机器每天DBC收益计算公式：',
      block_text2: '整机算力值/全网总算力值*547,945*DBC价格*地域系数**1.3（被租用算力值增加30%） - 整机算力值*地域系数*0.01693*30%（每个单位算力值每天收益是0.01693美元）',
      text3_13: '当前每天收益为: {usdt1} USD等值DBC = 链上奖励收益: {dbc} DBC(等值 {usdt2} USD 当前DBC价格: {dbc_price} USD) - 30%租金: {usdt3} USD等值DBC',
      ...
      req_text1: 'DLC租用机器的用户随时可以发起举报，举报的时候需要质押10000个DLC，当举报的时候，举报的人举报信息会进入DBC的验证人验证流程',
      req_title2: '六.惩罚机制(基于DBC AI公链的EVM兼容的 DLC)',
      req_text2: 'DLC举报被验证后，如果验证有问题，则按照规则扣除DBC质押币，DLC最多只扣除1万个DLC币，不扣除更多, 这些质押币全部奖励给举报用户',
      req_title3: '七.支付以及销毁机制 (基于DBC AI公链的EVM兼容的 DLC)',
      req_text3: '用户支付DLC(DLC网络中的GPU价格比DBC网络中GPU的价格上浮20％），获得机器的租用权利，用户支付的DLC 100%进入销毁智能合约，销毁智能合约可以查询每笔支付和销毁记录',
    }
  },
  short_rule: {
    title: '短租模式竞赛规则',
    ...
    cont5: {
      title: '四.奖励机制 （基于DBC AI公链的EVM兼容的DLC，浏览器地址：https://test.dbcscan.io/）',
      text1: '猎户座算力竞赛的个人短租GPU机器Token奖励总数为10亿枚，时间持续180天。竞赛总共分为3次。每次间隔5-10天时间。',
      text2: '竞赛一：每个类型GPU数量只有达到500个，竞赛才开始正式启动。',
      text3: '60天，每天300万枚，10％立马得到，90％ 180天解锁，每天解锁0.5%。',
      text4: '竞赛二：每个类型GPU数量只有达到1000个，竞赛才开始正式启动。',
      text5: '60天，每天400万枚，10％立马得到，90％ 180天解锁，每天解锁0.5%。',
      text6: '竞赛三：每个类型GPU数量只有达到2000个，竞赛才开始正式启动。',
      text7: '60天， 每天 966.5万枚，10％立马得到，90％ 180天解锁，每天解锁0.5%。',
      text8: 'Listing交易所后，测试币可以1：1兑换主网币',
      text9: 'DeepLink采用非线性奖励算法',

      text10: '这个系统的奖励每天的总token数量是固定的，机器获得奖励与机器的算力值X以及机器加入挖矿的有效时间Y,以及机器的质押Token数量有关系，质押的Token越多奖励越多，但是不能呈现线性增长关系。0-1000个币奖励基数是一样的，当超过1000个币奖励与函数呈现关联.如果机器处于被租用状态算力值增加30%  ',
      text11: '奖励计算公式：',
      text12: 'R：每天的总奖励Token数量（固定值）',
      text13: '第i台机器的质押Token数量',
      text14: '第i台机器的算力值',
      ...
      title1: '质押数量：',
      text1_1: '1）质押时可任意选择质押数量或者不质押',
      text1_2: '2）质押数量在0-1000个，获得奖励的数量一样，超过1000后非线形增长',
      text1_3: '3）当质押数量不足1000时，每次提取奖励都会先沉淀为质押，直到满足1000',
      text1_4: '4）10000个币的奖励是质押1000个币的1.23026倍，即大约增加了23.03%，10万个币的奖励是质押1000个币的1.46052倍，即大约增加了46.05%，质押100万个币的奖励是质押1000个币的1.69078倍，即大约增加了69.08%，质押1000万个币的奖励是质押1000个币的1.92103倍，即大约增加了92.10%',
      ...
      req_title1: '五. 在线检测机制',
      req_text1_1: '机器常驻有DBC AI公链的检测机制，上报信息给DBC分布式检测集群，用来判断机器是否处于在线状态，当不处于在线状态则扣除在线奖励。如果在用户使用过程中出现中断则扣除DLC给用户。惩罚的数量限制：',
